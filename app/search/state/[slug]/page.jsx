...
export default function StateSearchPage() {
  ...
  const stateName = params.slug ? parseSlug(params.slug) : ""

  useEffect(() => {
    if (stateName) {
      fetchEvents()
    }
  }, [stateName, currentPage, selectedType, sortBy])
  ...
  const fetchEvents = async () => {
    ...
    try {
      // Build filter for state search
      let filter = buildSearchFilter("", stateName, "state")
      ...
      console.log("State search filter:", filter)
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    className="h-12 shadow border-1 bg-white focus:bg-white transition-all duration-300 rounded-xl text-base placeholder:text-gray-400 focus:ring-2 focus:ring-blue-500/20"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-sm font-medium text-gray-700">Event Type</label>
                  <Select onValueChange={setSelectedType}>
                    <SelectTrigger className="h-12 shadow border-1 w-full bg-white focus:bg-white transition-all duration-300 rounded-xl text-base focus:ring-2 focus:ring-blue-500/20">
        ...
        {loading ? (
          ...
          >
            <div className="bg-white/60 backdrop-blur-sm rounded-3xl border border-white/30 p-12 max-w-md mx-auto">
              ...
              <h4 className="text-xl font-medium text-gray-900 mb-3">No events found in {stateName}</h4>
              <p className="text-gray-600 font-light mb-6">Try browsing events in other states or check back later</p>
              <Link href="/">
                  ...
                  return (
                    ...
                    >
                      <Link href={`/event/${eventSlug}`}>
                        <div className="bg-white backdrop-blur-sm rounded-2xl border-1 sh overflow-hidden transition-all duration-500 hover:shadow-2xl h-full">
